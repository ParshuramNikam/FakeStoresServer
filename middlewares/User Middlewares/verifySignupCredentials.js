...
import apiKeyGenerator from "../../helpers/apiKeyGenerator.js";
...
const verifySignupCredentials = async (req, res, next) => {
    try {
        ...
        if (isAlreadyRegistered && isAlreadyRegistered.verified) {
            ...
            return res.status(200).redirect(`${process.env.FRONTEND_BASE_URL}/user/login`);
        }
...
export default verifySignupCredentials;